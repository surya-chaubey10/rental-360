...
class SuperadminUserController extends Controller
{
           ...
           if($menuid){
              ...
              foreach($menuid as $key => $menu){
                ...
                //   if(isset($url[$submenuid])){
                  ...
                  $superadminUserPermission->menu_id = $menu;
                  $superadminUserPermission->user_id = $admin_user->id;
                  $superadminUserPermission->view_url = $request->read;
                  $superadminUserPermission->edit_url = $request->edit;
                  $superadminUserPermission->create_url = $request->create;
                  $superadminUserPermission->delete_url = $request->delete;
                    if (is_object($menu[$key])) {

                        foreach ($menu[$key] as $key => $submenu) {
                            $superadminUserPermission->menu_id = $menu;
                            $superadminUserPermission->sub_menu_id = $submenu;
                            $superadminUserPermission->view_url = $request->read;
                            $superadminUserPermission->edit_url = $request->edit;
                            $superadminUserPermission->create_url = $request->create;
                            $superadminUserPermission->delete_url = $request->delete;
                    ...
                    //   $superadminUserPermission->created_user =  Auth::user()->id;
  ...
  public function edit($uuid)
    ...
    public function all_submenu($role_type) {
         $all_submenu=SuperadminRoleMenu::with('role_sub_menu')->where('role_id',$role_type)
         ->get();
        $html='';
            ...
            foreach($all_submenu as $key=> $submenu){
                ...
                if ($submenu) {
                    $html .= '<td class="text-start">' . $submenu->admin_menu_name . '</td>
                            ...
                            <input type="hidden" id="menu_id" name="menu_id[]" value="' . $submenu->id . '">
                            ';
                    ...
                    // foreach ($submenu as $key2 => $data) {
                        ...
                        <input class="form-check-input check_permission" name="read" type="checkbox" value="read">
                        </div>
                        </td><td>
                        <div class="d-flex justify-content-center">
                        <input class="form-check-input check_permission" name="create" type="checkbox" value="create">
                        </div>
                        </td><td>
                        <div class="d-flex justify-content-center">
                        <input class="form-check-input check_permission" name="edit" type="checkbox" value="edit">
                        </div>
                        </td><td>
                        <div class="d-flex justify-content-center">
                        <input class="form-check-input check_permission" name="delete" type="checkbox" value="delete">
                        </div>
                        </td>';
                    ...
                    if (is_object($submenu->role_sub_menu)) {
                        foreach ($submenu->role_sub_menu as $key => $submenu1) {
                            $html .= '<tr>';

                            if ($submenu1) {
                                $html .= '<td class="text-start">' . $submenu1->admin_submenu_name . '</td>
                                        <input type="hidden" id="role_id" name="role_id" value="' . $role_type . '">
                                        <input type="hidden" id="menu_id" name="submenu_id[]" value="' . $submenu1->id . '">
                                        ';
...
//   {
//       $all_submenu=SuperadminRoleMenu::select('*')->with('role_sub_menu')
