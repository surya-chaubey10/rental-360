...
$(document).ready(function(){
    ...
    $('#add').click(function(){
        ...
        else
        {
            ...
            $.ajax({
                url:"../storeadmin/check-unique-bank-numbers",
                method:"POST",
   ...
   $('#save').click(function()
   {
    ...
    //    const ot_document4 = $('#ot_document4').val();
       ...
       const payout_day = $('#payout_day').val();  
        ...
        // formData.append('no_of_month', no_of_month);    
        ...
        //Seding Ajax Requrest for creating navbar menu
        $.ajax({
            url:"../storeadmin/create-process",
            method:"POST",
            data:formData,
            contentType:false,
            processData:false,
            ...
            success:function(data)
            {
               ...
               if(data == "company_logo")
                {
                    $('#company_logo_error').html("File type must be jpeg,png,jpg,gif,svg and size less than 2MB.");
                    return $('#company_logo').focus();
                }
                else if(data == "admin_email")
                {
                    $('#admin_email_error').html("Email already exists.");
                    return $('#admin_email').focus();
                }
                else{  
                if (data.status === true) {
                    ...
                    window.location.href = "../storeadmin/company-list";
                }
   ...
   $('#generate_company_prefix').click(function(){
        ...
        $('#auto_renewal').change(function(){
            (this.checked 
                ...
                : $('#end_billing_date').prop('readonly','readonly')
            )
        });
    ...
    $('#billing_plan').change(function(){
        ...
        $.ajax({
            url:"../storeadmin/subscription_plan_details/"+$plan_id,
            method:"GET",
            contentType:false,
            processData:false,
            cache:false,
            ...
            { 
                ...
                //     $('#payement_gateway_amount').val(explode);
                //     //$('.pga'+propVal).removeAttr('readonly');
                // });
