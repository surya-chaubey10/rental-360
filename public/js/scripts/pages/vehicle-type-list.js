...
$(function () {
  ...
  $.ajaxSetup({
   ...
   var dtUserTable = $('.vehicle-type-table'),  // here
    formBlock = $('.btn-form-block'),
    formSection = $('.form-block'),  
    newUserForm = $('.add-vehicle-type'),  //here  
    select = $('.select2'), 
  ...
  if (dtUserTable.length) {
    dtUserTable.DataTable({
      ajax: assetPath + "data/vehicle_type-json/" + org_id + "_vehicle_type-list.json", // JSON file to add data
      // ajax: assetPath + 'data/vehicle_type.json',  
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $name = full['type_name'], 
              $image = full['type_image'];
            if ($image) {
              // For Avatar image 
              var $output =
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return "<span class='text-truncate align-middle'>"  + $type_name + '</span>';
          }
        },
        {
          targets: 3,
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['type_name'];
            }
          }),
          type: 'column',
  ...
  newUserForm.on('submit', function (e) {
    if (!e.isDefaultPrevented()) {
        ...
        if (formBlock.length && formSection.length) {
          ...
          let formData = new FormData($('#addvehicle_type')[0])
       $.ajax({
              url: 'store-vehicle-type', // JSON file to add data,
              type: 'POST',
              dataType: 'json',
              data: formData,
              contentType: false,
              ...
              success: function (data) {
                  ...
                  if (data.status === true) {
                      ...
                      window.location = "/vehicle-type-list";  
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    function deleteRecord(value_id) {
      $.ajax({
        url: '../vehicle-type-delete'+'/'+value_id, // JSON file to add data,
        type: 'get',
        dataType: 'json',
        contentType: false,
        processData: false,
